...
```
Make a new directory called `RSEM_Copper_Trinity_LT` and move the output files into this directory to keep them separate from those in step 2.
...
```
The mapping rates ranged from 85-91% for this assembly. Make a new directory called `RSEM_CGH_Trinity_CL` and move the output files into this directory to keep them separate from those in step 1.

## Build counts matrix and perform differential expression analysis

These steps need to be performed SEPARATELY using the output files from each assembly completed above.
