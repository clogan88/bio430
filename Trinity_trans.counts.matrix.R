...
if (file.exists("Trinity_trans.counts.matrix.RData") && ! NO_REUSE) {
    ...
    primary_data = read.table("Trinity_trans.counts.matrix", header=T, com='', sep="\t", row.names=1, check.names=F)
...
data = primary_data
samples_data = read.table("samples_file", header=F, check.names=F, fill=T)
...
write.table(data, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.dat", quote=F, sep='	');
pdf("Trinity_trans.counts.matrix.minRow10.CPM.log2.principal_components.pdf")
data = as.matrix(data)
...
write.table(pca$rotation, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.PCA.loadings", quote=F, sep="	")
write.table(pca$x, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.PCA.scores", quote=F, sep="	")
